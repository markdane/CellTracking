...
well <- "A2"
field <- 1
filename <- paste(plateID,well, field, "Points.csv",sep = "_")
out_filename <- paste(plateID,well, field, "intensities.csv",sep = "_")

df_input <- read_csv(paste0("MTrackJ/Data/",filename), col_types = cols(
  Nr = col_double(),
  TID = col_double(),
  PID = col_double(),
  `x [pixel]` = col_double(),
...
####Debug problem with minutes vs seconds
...
res <- write_csv(df_intensities,
                 paste0("MTrackJ/Data/",out_filename),
                 na = "")
```

...
df_orphans <-  dir("Data", pattern = "LI802303.*_orphans.csv$", full.names = TRUE) %>%
  map(read_csv,
      col_types = cols(
        tmin = col_character(),
        TID = col_character(),
  ...
  map(tibble) %>%
  bind_rows()
write_csv(df_orphans, "LI802303_orphans.csv")
...
df <- df_input %>%
  select(TID, lineage, field, well, treatment, average_velocity) %>%
  group_by(treatment) %>%
...
df <- df_input %>%
  select(TID, lineage, field, well, treatment, average_velocity) %>%
  group_by(treatment, well, field, TID) %>%
  summarise(average_velocity = mean(average_velocity, na.rm = TRUE)) %>%
  ungroup()

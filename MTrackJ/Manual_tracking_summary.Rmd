...
df <- single_cell_lineage_T0_rel %>%
  ...
  distinct() %>%
  arrange(thour)
#Save this summary data to add to the integrated analysis
res <- write_csv(df, "MDD_migration_distance_lineage_T0.csv")

p <- ggplot(single_cell_lineage_T0_rel, aes(x = thour, y = lineage_length,  group = lineage_key, colour = factor(lineage_col))) +
  geom_point(data = mitotic_events, size = .3, alpha = .8) +
  geom_path(size = .4, alpha = .8) +
  scale_x_continuous(breaks = c(0,12,24, 36, 48)) +
...
#p <- p + 
p <- p + geom_path(data = df, aes(x = thour, y = lineage_length_mean), color = "black", group = 1, alpha = .8, size = .8)
p

if(create_pdfs){
  pdf("MDD_migration_distance_size_pt4.pdf", useDingbats = FALSE, width = 8, height = 2)
  print(p)
  res <- dev.off()
}

...
foo <- df_input %>%
  mutate(key = paste0(well_field,"_",TID)) %>%
  select(key, motherID, treatment, well, TID) %>%
  distinct()
...
df <- df_input %>%
  group_by(treatment, tmin, well, field) %>%
  summarise(distance = mean(distance)) %>%
  ungroup()
...
```{r relative_migration, fig.width=8, fig.height=7}
...
single_daughter_TIDs <- T0_rel %>%
  mutate(key = paste0(well, "_",field, "_", lineage)) %>%
  filter(key %in% lineages$key) %>%
  select(TID, generation, key, treatment, lineage) %>%
  distinct() %>%
...
p <- ggplot(df, aes(x_rel, y_rel, colour = factor(lineage))) +
  ...
  #coord_cartesian(xlim = c(-900, 900), ylim = c(-900, 900)) +
  guides(colour = FALSE) +
  labs(title = "Relative motion of equal number of lineages, one daughter")+
  theme_bw()+
  theme(strip.text = element_text(size = 12),
        strip.background = element_rect(fill = "transparent"),
        axis.title = element_blank(),
...
df <- df_daughters_gen1_rel %>%
  select(well, field, TID, treatment, lineage, generation, tmin, x_rel, y_rel) %>%
  bind_rows(df_gen1_T0_rel)

#####debug here... problem with mixed generations in the lineage at the same time
foo <- df %>%
  filter(well=="B03",
         lineage == 2,
         field == "D5_3")
...
p <- ggplot(df, aes(x_rel, y_rel, colour = factor(generation))) +
  #geom_point(size = .1, alpha = .8) +
  geom_path(size = .4, alpha = .8) +
  scale_colour_manual(values = cols) +
  #coord_cartesian(xlim = c(-900, 900), ylim = c(-900, 900)) +
  #guides(colour = FALSE) +
  labs(title = "Relative motion of equal number of lineages, one daughter",
       subtitle = "colored by generation")+
  theme_bw()+
  theme(strip.text = element_text(size = 12),
        strip.background = element_rect(fill = "transparent"),
        axis.title = element_blank(),

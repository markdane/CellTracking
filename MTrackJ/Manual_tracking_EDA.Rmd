...
#path <- "/Users/dane/Documents/CellTracking/MTrackJ/Data/LI802303_A02_X_2_Points.csv"
tmin_period = 30
```

```{r}

df_input_raw <- read_csv(paste0(path), col_types = cols(
  Nr = col_double(),
  TID = col_double(),
  PID = col_double(),
  `x [pixel]` = col_double(),
...
wellName <- str_extract(path, "[AB]0[1234]")
fieldName <- str_extract(path, "[AB]0[1234].*_Points") %>%
  str_remove_all("[AB]0[1234]_|_Points")
...
df <- df_lineage_raw %>%
  ...
  summarise(distance_mean = mean(distance),
            ...
            average_speed = distance_mean/tmin_period) %>%
  ungroup() %>%
...
df <- df_lineage %>%
  select(TID, distance_mean, distance_total, average_speed, treatment,generation, lineage) %>%
  distinct()
...
p <- ggplot(df, aes(x = TID, y = average_speed,fill = factor(lineage))) +
  geom_col() +
  guides(fill = FALSE)
p

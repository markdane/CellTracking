...
function registerStacks(dir_list) {	
  for (i=0; i<dir_list.length; i++) {
      if(matches(dir_list[i], "[A-Z][1-9]_[1-9]/")) {
		...
		rename("P_Reg");
      	//merge registered P and R contrast stacks
      	//debug saving as seprate channel tiffs
      	print("Merging channels of registered R and P stacks in well_location " +  dir + "/" + dir_list[i]);
      	run("Merge Channels...", "c1=R_Reg c4=P_Reg create");
       	selectWindow("Composite");
      	rename("P_Reg");
      	selectWindow("P_Reg");
		run("Flatten");
		print("Saving merged RGB for " +  dir + dir_list[i]);
		run("Image Sequence... ", "format=TIFF save=" +  dir + "/" + dir_list[i] + "/P_Reg");
  		while (nImages>0) {
  			selectImage(nImages);
  			close();
  			} 

...
generate_summary_plots <- function(df){
  ...
  df_mean <- df %>%
    ...
    group_by(hours, treatment, drugs, Drug1Concentration, Drug1ConcentrationRank) %>%
  ...
  df_mean_select <- df_mean %>%
    filter(hours %in% c(5, 24, 48, 72, 96)) %>%
  ...
  p_n_t_T0_mean <- ggplot(df_mean, aes(x = hours, y = cell_count_norm_T0, color = treatment, group = Drug1Concentration)) +
    ...
    labs(title = "HCC1143 Cell counts for each treatment (T0 normed)",
...
names(drug_cols) <- unique(df_mean_select$hours)
  ...
  df_mean_select_0 <- df_mean_select %>%
    select(drugs, hours) %>%
    distinct() %>%
    mutate(Drug1ConcentrationRank = 0,
...
p_cell_count_dose_response <- ggplot(df_mean_select_0, aes(x = Drug1ConcentrationRank, y = cell_count_norm_control, color = hours, group = hours)) +
  ...
  pdf(paste0(data_path,"HCC1143_summary.pdf"), width = 10, height = 4)
  print(p_n_t_T0_mean)
  print(p_G1_t_mean)
...
get_level_2_data <- function(plateID, data_path = "/home/exacloud/gscratch/HeiserLab/images/", pipeline_name = "CKn"){
  df <- read_csv(paste0(data_path, plateID,"/Analysis/",pipeline_name,"/",plateID,"_level_2.csv"), show_col_types = FALSE)
}
...
hcc1143_l2 <- map_df(plateIDs, get_level_2_data, data_path = data_path) %>%
  mutate(hours = elapsed_minutes/60)
res <- generate_summary_plots(hcc1143_l2)

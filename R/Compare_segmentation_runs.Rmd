---
title: "Compare segmentation runs"
output:
  html_document: null
...
```{r functions}
get_data <- function(plateID, data_path, pipeline_name = "PC2", data_level = "1"){
  ...
  if(str_detect(pipeline_name, "PC")){
    reporter = "NR"
  } else {
    reporter = "CC"
  }
...
new_data <- get_data(plateID = "AU02001", data_path=data_path, pipeline_name = "PC2", data_level = "1") %>%
  filter(Nuclei_area > area_threshold_min,
         Nuclei_area < area_threshold_max) %>%
  count(well, field, time_slice, name = "cell_count_new") %>%
  arrange(time_slice) 

baseline_data <- get_data(plateID = "AU02001", data_path=data_path, pipeline_name = "PC", data_level = "1") %>%
  filter(Nuclei_area > area_threshold_min,
         Nuclei_area < area_threshold_max) %>%
  count(well, field, time_slice, name = "cell_count_baseline") %>%
  arrange(time_slice) 

# ilastik_data <- get_data(plateID = "AU02001", data_path=data_path, pipeline_name = "P", data_level = "1") %>%
#   filter(Nuclei_area > area_threshold_min,
#          Nuclei_area < area_threshold_max) %>%
#   count(well, field, time_slice, name = "cell_count_ilastik") %>%
#   arrange(time_slice)

cell_count_df <- inner_join(new_data, baseline_data, by = c("well", "field", "time_slice")) %>%
  arrange(well, field, time_slice) %>%
  mutate(cell_count_new_log2 = log2(cell_count_new),
         cell_count_baseline_log2 = log2(cell_count_baseline),
...
df %>%
  filter(well == wells) %>% 
  nest(data = c(time_slice, cell_count_baseline, day, hour, minute, elapsed_minutes, 
                cell_count_new, cell_count_baseline_log2, cell_count_new_log2)) %>%
...
evaluate_cell_counts("D2","new")
evaluate_cell_counts("D2","baseline")
...
wells <- "D2"
p <- ggplot(cell_count_df %>%filter(well == wells), aes(x = cell_count_new, y = cell_count_baseline)) +
  geom_point(size = .5) +  
  facet_wrap(~field)+
  labs(title =wells) +
  theme(strip.text = element_text(size = 5)) +

...
plateID <- "AU01401"
if(!file.exists(paste0("../data/",plateID,"_level_1.csv"))){
  raw_data <- dir("../data", pattern = plateID,full.names = TRUE)[1:4] %>%
...
write_csv(field_data,paste0("../data/",plateID,"_level_1.csv"))

} else {
  field_data <- read_csv(paste0("../data/",plateID,"_level_1.csv"))
}


```

```{r qa_check, fig.height=20}
# 
# p_area <- ggplot(data_ann, aes(x = Nuclei_CL_NR_area, fill = factor(field))) +
#   geom_density(alpha = .3) +
#   coord_cartesian(xlim = c(125,600))+
#   labs(title = "Cell area density in each field",
#        xlab ="area") +
#   theme_bw()+

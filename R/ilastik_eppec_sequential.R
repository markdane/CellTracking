...
pwd <- getwd()
dirs <- paste0(pwd,"/",dir(pattern = "[[:alnum:]]*_[[:digit:]]*"))[5]
#dir_path = "A3_1"
...
res <- lapply(dirs, function(dir_path){
  ...
  system(paste0('srun -c 8 /home/users/dane/ilastik-1.3.2b3-Linux/run_ilastik.sh --headless --readonly=True --project=/graylab/share/dane/CellTracking/HeLa_Tracking.ilp --raw_data="',dir_path,'/TxRed_Reg/',well_location,'_TxRed_Scene1Interval*.tif" --prediction_maps=',dir_path,'/TxRed_Reg/Probabilities.h5 --export_source="Plugin" --export_plugin="CSV-Table"'))
  
  system(paste0('srun -c 8 /home/users/dane/ilastik-1.3.2b3-Linux/run_ilastik.sh --headless --readonly=True --project=/graylab/share/dane/CellTracking/HeLa_Tracking.ilp --raw_data="',dir_path,'/GFP_Reg/',well_location,'_GFP_Scene1Interval*.tif" --prediction_maps=',dir_path,'/TxRed_Reg/Probabilities.h5 --export_source="Plugin" --export_plugin="CSV-Table"'))
})

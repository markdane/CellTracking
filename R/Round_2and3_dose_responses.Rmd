...
```{r functions}
get_l2_data <- function(plateID, data_path, pipeline_name = "CKn"){
...
pipeline_name <- "CKn"
plateIDs <- c("AU01701","AU01801","AU01802","AU01902","AU02001","AU02002","AU02301","AU02401", "AU02501", "AU02801", "AU03001", "AU03201", "AU03401", "AU03501", "AU03601", "AU03701")
...
l2 <- map_df(plateIDs[c(13, 14,16)], get_l2_data, data_path = data_path) %>%
...
p_T0_count <- ggplot(df, aes(x = elapsed_hours, y = cell_count_norm_T0, color = interaction(well,field))) +
  geom_path() +
  scale_x_continuous(breaks = c(0,24, 48, 72, 96))+
  #scale_color_manual(values = cols) +
  labs(title = "Cell counts for all controls") +
  xlab("hours") +
  ylab("Cell count (T0 normed)") +
  guides(color ="none") +
  facet_grid(plateID~well)+
  theme(strip.text = element_text(size = 5)) +
  theme_bw()
p_T0_count

p_G1 <- ggplot(df, aes(x = elapsed_hours, y = G1_proportion, color = interaction(well,field))) +
  geom_path() +
  scale_x_continuous(breaks = c(0,24, 48, 72, 96))+
  #scale_color_manual(values = cols) +
  labs(title = "G1 proportion for all controls") +
  xlab("hours") +
  ylab("G1 proportion") +
  guides(color ="none") +
  facet_grid(plateID~well)+
...
pdf(file = "../plots/AU565_rounds2and3_controls_G1.pdf",height = 12)
print(p_T0_count)
print(p_G1)
res <- dev.off()

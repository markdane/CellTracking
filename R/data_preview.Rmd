...
process_plate<- function(plateID, ...){
  ...
  #process track files
  res <- map(dir(paste0(data_path,plateID), pattern = "tracks.csv", recursive = TRUE, full.names = TRUE), process_tracks_file)
...
df <- l1 %>%
  select(well, field, slice, time_slice, elapsed_minutes, day,hour, minute, treatment, label, begins, ends, parent, length, Drug1, Drug1Concentration, Cell_PC_CC_mean_intensity_ratio,cell_cycle_state) %>%
  filter(field ==1) %>%
  mutate()
...
gen_0_lineages <- ggplot(gen_0, aes(x = elapsed_minutes, y = label, color = cell_cycle_state)) +
  geom_path(size = .5) +
...
pdf(paste0("../plots/",plateID,"_lineage_plots.pdf"), height = 12)
print(gen_0_lineages)
res <- dev.off()

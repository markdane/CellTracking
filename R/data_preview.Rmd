...
generate_plots <- function(plateID, l2){
  ...
  l2_mean <- l2 %>%
    group_by(well, elapsed_minutes, treatment, drugs, Drug1Concentration) %>%
    summarise(cell_count_norm_T0 = mean(cell_count_norm_T0),
              n = mean(n))

      p_n_t_T0_mean <- ggplot(l2_mean, aes(x = elapsed_minutes, y = cell_count_norm_T0, color = treatment, group = interaction(well, Drug1Concentration))) +
    geom_path() +
    scale_y_continuous(trans = 'log2') +
...
res <- map(c("AU01401", "AU01601", "AU01701"), process_plate)

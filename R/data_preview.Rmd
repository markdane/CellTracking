...
create_level_2_data <- function(plateID, data_path = "/home/exacloud/gscratch/HeiserLab/images/", pipeline_name = "PC"){
  ...
  level_1_data_path <- paste0(data_path,plateID,"/Analysis/",pipeline_name,"/intermediate_files/",plateID,"_level_1.csv")
  level_2_data_path <- paste0(data_path,plateID,"/Analysis/",pipeline_name,"/intermediate_files/",plateID,"_level_2.csv")
...
cols <- c("BEZ235_1" = "cornflowerblue", "Cabozantinib_50" = "cornflowerblue", "Trametinib_50" = "cornflowerblue",
  ...
  if(!file.exists(level_1_data_path)){
    ...
    l1 <- map(dir(paste0(data_path,plateID,"/Analysis/",pipeline_name,"/intermediate_files/"),pattern = paste0(plateID,"_.*_level_1.csv"), full.names = TRUE), read_csv,
              col_types =cols(.default = "c",
                              ...1 = col_double(),
                              label...2 = col_double(),
                              Nuclei_PC_CC_area = col_double(),
                              Nuclei_PC_CC_eccentricity = col_double(),
                              Nuclei_PC_CC_mean_intensity = col_double(),
                              Nuclei_PC_CC_max_intensity = col_double(),
                              Nuclei_PC_CC_min_intensity = col_double(),
                              image = col_double(),
                              well = col_character(),
                              field = col_double(),
                              time_slice = col_character(),
                              label...12 = col_double(),
                              Cyto_PC_CC_mean_intensity = col_double(),
                              Cyto_PC_CC_max_intensity = col_double(),
                              Cyto_PC_CC_min_intensity = col_double(),
                              Cell_PC_CC_mean_intensity_ratio = col_double(),
                              Cell_PC_CC_max_intensity_ratio = col_double(),
                              Cell_PC_CC_min_intensity_ratio = col_double()
              )) %>%
      bind_rows() %>%
      mutate(day = str_remove(time_slice, "d.*"),
             ...
             cell_cycle_state = case_when(Cell_PC_CC_mean_intensity_ratio <= g1_threshold ~"G1",
                                          Cell_PC_CC_mean_intensity_ratio > g1_threshold ~"S/G2")) %>%
    ...
    l2 <- l1 %>%
      ...
      drop_na() %>%
      summarise(n = n(),
                Nuclei_PC_CC_mean_intensity=mean(Nuclei_PC_CC_mean_intensity),
                Cyto_PC_CC_mean_intensity=mean(Cyto_PC_CC_mean_intensity),
                Cell_PC_CC_mean_intensity_ratio=mean(Cell_PC_CC_mean_intensity_ratio),
                Cell_PC_CC_max_intensity_ratio=mean(Cell_PC_CC_max_intensity_ratio),
                Cell_PC_CC_min_intensity_ratio=mean(Cell_PC_CC_min_intensity_ratio),
...
#hack to deal with bortexomib_50 used once as a low dosage and another time as a high doasge
  ...
  p_CC <- ggplot(l2, aes(x = elapsed_minutes, y = Nuclei_PC_CC_mean_intensity, color = factor(field))) +
    geom_path() +
    stat_summary(fun=mean, aes(group=1), geom="line", colour="blue") +
  ...
  p_CC_t <- ggplot(l2, aes(x = elapsed_minutes, y = Nuclei_PC_CC_mean_intensity, color = treatment, group = interaction(field, well))) +
    geom_path() +
    stat_summary(fun=mean, aes(group=1), geom="line", colour="blue") +
    labs(title = "Cell cycle intensity in the nuclei, mean",
         xlab = "minutes",
  ...
  p_cyto_CC <- ggplot(l2, aes(x = elapsed_minutes, y = Cyto_PC_CC_mean_intensity, color = factor(field))) +
    geom_path() +
    stat_summary(fun=mean, aes(group=1), geom="line", colour="blue") +
  ...
  p_cyto_CC_t <- ggplot(l2, aes(x = elapsed_minutes, y = Cyto_PC_CC_mean_intensity, color = treatment, group = interaction(field, well))) +
    geom_path() +
    stat_summary(fun=mean, aes(group=1), geom="line", colour="blue") +
    labs(title = "Cell cycle intensity in the cytoplasm, mean",
         xlab = "minutes",
  ...
  p_cyto_CC_ratio <- ggplot(l2, aes(x = elapsed_minutes, y = Cell_PC_CC_mean_intensity_ratio, color = factor(field))) +
    geom_path() +
    stat_summary(fun=mean, aes(group=1), geom="line", colour="blue") +
  ...
  p_cyto_CC_t_ratio <- ggplot(l2, aes(x = elapsed_minutes, y = Cell_PC_CC_mean_intensity_ratio, color = treatment, group = interaction(field, well))) +
    geom_path() +
    stat_summary(fun=mean, aes(group=1), geom="line", colour="blue") +
    labs(title = "Cell cycle intensity in the cytoplasm/nuclei, mean",
         xlab = "minutes",
  ...
  p_densities <- ggplot(df, aes(Cell_PC_CC_mean_intensity_ratio, fill = treatment, color = treatment)) +
  ...
  # p <- ggplot(df, aes(Cell_PC_CC_mean_intensity_ratio, fill = elapsed_hours, color = elapsed_hours)) +
  #   geom_density(alpha = .2)+
...
compare_to_CP <- function(plateID, l2){
  ...
  p_PC_CP <- ggplot(cell_counts_PC_CP, aes(x = elapsed_minutes, y = Cell_count, color = pipeline, shape = pipeline)) +
  ...
  cell_counts_PC_CP_wide <- cell_counts_PC_CP %>%
    pivot_wider(names_from = pipeline, values_from = Cell_count)
  
  p_PIvsCP <- ggplot(cell_counts_PC_CP_wide, aes(x = CellProfiler, y = python_ilastik)) +
    geom_point(size = .5, alpha = .8) +
...
res <- map(c("AU02001"), process_plate)

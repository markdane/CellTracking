...
#   system(paste0('srun -c 8 /home/users/dane/ilastik-1.3.2b3-Linux/run_ilastik.sh --headless --project=/graylab/share/dane/CellTracking/IncuCyte_pixels.ilp --raw_data="',dir_path,'/P_Reg/P_Reg*.tif" --output_format="hdf5" --output_filename_format=',dir_path,'/R_Reg/Probabilities.h5'),
#          wait=TRUE)
# })
# 
...
res <- lapply(dirs, function(dir_path){
  ...
  well_location <- str_remove(dir_path,".*/")
  system(paste0('srun -c 4 /home/users/dane/ilastik-1.3.2b3-Linux/run_ilastik.sh --headless --readonly=True --project=/graylab/share/dane/CellTracking/IncuCyte_tracking.ilp --raw_data="',dir_path,'/R_Reg/R_Reg*.tif" --prediction_maps=',dir_path,'/R_Reg/Probabilities.h5 --export_source="Plugin" --export_plugin="CSV-Table"'),
         wait=FALSE)
})

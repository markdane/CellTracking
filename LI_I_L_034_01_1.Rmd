...
clean_processed_data <- processing1 %>%
  group_by(Well) %>%
  select(Location_trackId, Mean_Intensity_0_reporter_sum, Well) %>%
  distinct() %>%
  arrange(desc(Mean_Intensity_0_reporter_sum)) %>%
  group_by(Well) %>%
  mutate(Mean_Intensity_0_reporter_sum_rank = row_number()) %>%
  ungroup() %>%
  select(Location_trackId, Mean_Intensity_0_reporter_sum_rank) %>%
...
loProb <- quantile(clean_processed_data$Mean_Intensity_0_reporter, probs=.01)
highProb <- quantile(clean_processed_data$Mean_Intensity_0_reporter, probs=.99)

df <- clean_processed_data %>%
  #filter(Well=="A1") %>%
  mutate(Mean_Intensity_0_reporter = scales::squish(Mean_Intensity_0_reporter, range = c(loProb, highProb)))
  

p <- ggplot(df, aes(x=Time, y = Mean_Intensity_0_reporter_sum_rank,
                    fill = Mean_Intensity_0_reporter,
                    colour = Mean_Intensity_0_reporter)) + 
  geom_tile() + 
...
df <- clean_processed_data %>%
  ...
  mutate(Mean_Intensity_0_reporter_T0norm = scales::squish(Mean_Intensity_0_reporter, range = c(loProb, highProb)))

p <- ggplot(df, aes(x = Mean_Intensity_0_reporter, fill = factor(Ligand_1))) +
...
p <- ggplot(df, aes(x = factor(Ligand_1), y = Mean_Intensity_0_reporter, fill = factor(Ligand_1))) +
  geom_violin(alpha=.3) +
  labs(x = "Ligand",
...
p <- ggplot(clean_data, aes(x = Time, y = Mean_Intensity_0_reporter, group = factor(Location_trackId))) +
  geom_line(colour="lightgray", alpha=.8, size=.4) +
  guides(colour=FALSE) +
...
df <- clean_data_small %>%
  group_by(Location, Time) %>%
  summarise(Mean_Intensity_0_reporter = mean(Mean_Intensity_0_reporter),
            Ligand_1 = unique(Ligand_1)) %>%
  ungroup()

p <- ggplot(df, aes(x = Time, y = Mean_Intensity_0_reporter, colour=Location)) +
  geom_line() +
...
p <- ggplot(df, aes(x = Time, y = Mean_Intensity_0_reporter, colour=Well)) +
  geom_line() + 
...
p <- ggplot(df, aes(x = Time, y = Mean_Intensity_0_reporter, colour=factor(Ligand_1))) +
  geom_line() + 
    labs(title = "Reporter intensity dose responses",
       y = "Reporter intensity",
       x = unique(raw_data_metadata$Image_period_timeUnit),
...
clean_data_small <- clean_data_small %>%
  mutate(bb_min_0_reporter = Center_of_the_object_0-mean_bounding_box_dims[1],
         ...
         bb_min_0_reporter = pmax(0, bb_min_0_reporter),
         bb_max_0_reporter = pmin(1280, bb_max_0_reporter),

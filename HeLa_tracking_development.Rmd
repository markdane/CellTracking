...
p <- ggplot(df, aes(x = frame, y = log10(Mean_Intensity_0_nuclear_T0Norm), colour = factor(trackId_nuclear))) +
  ...
  theme(axis.title.y=element_blank(),
        ...
        axis.ticks.y=element_blank())+
  facet_wrap(~Location)
p <- p + geom_smooth(aes(x = frame,  y = log10(Mean_Intensity_0_nuclear_T0Norm)),method = "lm", colour="black")
p
...
p <- ggplot(df, aes(x = frame, y = log10(Mean_Intensity_0_reporter_T0Norm), colour = factor(trackId_nuclear))) +
  ...
  #       axis.ticks.y=element_blank())+
  facet_wrap(~Location)
p <- p + geom_smooth(aes(x = frame,  y = log10(Mean_Intensity_0_reporter_T0Norm)), colour="black", method="lm")
p
...
```{r cytoReporterIntensityCell, fig.width=8, fig.height=6}
df <- cyto_data 

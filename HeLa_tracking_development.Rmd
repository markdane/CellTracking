...
processing1 <- raw_data_metadata %>%
  ...
  mutate(Mean_Intensity_0_reporter_sum = sum(Mean_Intensity_0_reporter),
         ...
         Mean_Intensity_0_raw_reporter_T0 = Mean_Intensity_0_raw_reporter[frame==0],
...
```{r sampleImages, fig.height=12, fig.width=8}

#Get images to be displayed
#need to recreate path
df <- clean_data %>%
  filter(Time %in% c(9,30,60,90,120, 150, 177)) %>%
  select(Well, frame, Location) %>%
  distinct()
...
foo <- paste0(wldf$Well_Location_dir[10],"/GFP_Reg/",wldf$Location[10],"_GFP_Scene1Interval",wldf$frame[10],".tif") %>% 
  readTIFF()

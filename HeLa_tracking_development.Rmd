...
```{r reporterheatmap, fig.width=6, fig.height=9, eval=TRUE}
#Heatmap with time columns and location)trackId rows, colour based on reporter intensity
loProb <- quantile(clean_data$Mean_Intensity_0_reporter_T0norm, probs=.02)
highProb <- quantile(clean_data$Mean_Intensity_0_reporter_T0norm, probs=.98)
...
# for(well in unique(clean_data_small$Well)){
#   dm <- clean_data_small %>%
#     filter(Well == well) %>%
#     select(Location_trackId, Time, Mean_Intensity_0_reporter_T0norm, Mean_Intensity_0_reporter_sum) %>%
#     arrange(Time, desc(Mean_Intensity_0_reporter_sum), Location_trackId) %>%
#     select(Mean_Intensity_0_reporter_T0norm) %>%
#     unlist() %>%
#     matrix(ncol = length(unique(clean_data_small$Time)))
#   
#   colnames(dm) <- unique(clean_data_small$Time)
#   
#   hmo <- heatmap.2(dm,
#                    dendrogram = "none",
#                    Rowv=FALSE,
#                    Colv = FALSE,
#                    #RowSideColors = ggCols[cldf$KMeanFirstcl3[cellIntFirst$Cell]],
#                    trace="none",
#                    key = FALSE,
#                    col = spectralPal,
#                    labCol=FALSE,
#                    labRow = FALSE,
#                    main = "T0 normalized reporter intensity",
#                    ylab="Cells")
#}
```

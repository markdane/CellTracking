...
p <- ggplot(df, aes(x = Time, y = log10(Mean_Intensity_0_nuclear_T0Norm), colour = factor(trackId_nuclear))) +
  geom_line(alpha=.3, size=.8) +
    guides(colour=FALSE) +
...
p <- ggplot(df, aes(x = Time, y = log10(Mean_Intensity_0_reporter_T0Norm), colour = factor(trackId_nuclear))) +
  geom_line(alpha=.3, size=.8) +
  guides(colour=FALSE) +
